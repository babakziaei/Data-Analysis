...
/*
1. To begin, let's evaluate our growth in terms of volume. Kindly extract the overall number of sessions and orders, 
broken down by quarters since the business's inception. You have the discretion to handle the data for the most recent incomplete quarter.
*/ 
...
/*
2. Subsequently, let's demonstrate our efficiency enhancements. I am interested in seeing quarterly statistics 
since our inception, detailing the conversion rate from sessions to orders, the revenue per order, and the revenue per session. 
*/
...
/*
3. I am interested in understanding the growth of specific channels. Could you provide a quarterly breakdown of orders 
coming from Gsearch nonbrand, Bsearch nonbrand, overall brand search, organic search, and direct type-in?
*/

SELECT 
    YEAR(website_sessions.created_at) AS year,
    QUARTER(website_sessions.created_at) AS quarter, 
    COUNT(DISTINCT CASE WHEN utm_source = 'gsearch' AND utm_campaign = 'nonbrand' THEN orders.order_id ELSE NULL END) AS gsearch_nonbrand_order_count, 
    COUNT(DISTINCT CASE WHEN utm_source = 'bsearch' AND utm_campaign = 'nonbrand' THEN orders.order_id ELSE NULL END) AS bsearch_nonbrand_order_count, 
    COUNT(DISTINCT CASE WHEN utm_campaign = 'brand' THEN orders.order_id ELSE NULL END) AS overall_brand_search_order_count,
    COUNT(DISTINCT CASE WHEN utm_source IS NULL AND http_referer IS NOT NULL THEN orders.order_id ELSE NULL END) AS organic_search_order_count,
    COUNT(DISTINCT CASE WHEN utm_source IS NULL AND http_referer IS NULL THEN orders.order_id ELSE NULL END) AS direct_type_in_order_count
FROM website_sessions 
    LEFT JOIN orders
        ON website_sessions.website_session_id = orders.website_session_id
...
/*
4. Next, let's visualize the overall trends of the session-to-order conversion rate for the same channels, 
segmented by quarter. Also, make sure to highlight any periods of substantial improvements or enhancements.
*/
...
;
/*
5. Our product range has significantly expanded from the time we started with just one offering. Let's compile monthly statistics for each product's revenue and profit margins, as well as overall sales and revenue. Be observant and mark any patterns or trends you see related to different seasons or times of the year.
*/
